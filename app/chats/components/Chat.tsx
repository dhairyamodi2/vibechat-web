...
export const ChatBox : React.FC<ChatProps> = function( {chat, lastMessage}) {
    ...
    return (
        <div className={`${params && params.chatId ? 'hidden p-0 my-0 sm:flex' : ''} p-3 flex items-center my-3 rounded-xl ${params.chatId === chat.id ? 'bg-gray-200' : ''} hover:bg-gray-200 flex-1 cursor-pointer shadow-lg drop-shadow-md mx-2`} ref={lasref} onClick={handleClick}>
            ...
            <div className="flex-1 mx-3 my-1">
                <div className="flex flex-col">
                    ...
                    <span className="text-sm mt-0.5">{chat.messages.length === 0 ? 'Start a conversation' : chat.messages[chat.messages.length - 1].body?.substring(0,25) + '...'}</span>
                </div>    
            </div>
            <div >
                <span className="text-sm">{chat.messages.length !== 0 ? format(new Date(chat.messages[chat.messages.length-1].createdAt), 'p') : ''}</span>
            </div>
        </div>
    )
}